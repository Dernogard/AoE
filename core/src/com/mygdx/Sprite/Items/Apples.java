...
public class Apples extends Items {
    ...
    public Apples(AllLevels lvl, Rectangle bounds) {
        super(lvl, bounds);
        fixture.setUserData(this);
        setCategoryFilter(MyGdxGame.APPLE_BIT);

        texture = new TextureRegion(new Texture("resourse/Apple.png"), 0,0,64,64);
        setBounds(0,0,64 / MyGdxGame.PPM,64 / MyGdxGame.PPM);
        setPosition(body.getPosition().x - getWidth() / 2, body.getPosition().y  - getHeight() / 2);
        setRegion(texture);

        usedSound = Gdx.audio.newSound(Gdx.files.internal("Audio/Sound/smb_stomp.wav"));
